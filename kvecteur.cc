...
#include <map>
#include "iostream"
#if defined HAVE_SSTREAM || defined FXCG || defined TICE
#include <sstream>
#else
#include <strstream>
#endif
...
namespace giac {
  ...
  gen _peval(const gen & e,GIAC_CONTEXT){
    ...
    for (int i=0;i<signed(vals.size());++i){
      if (debug_infolevel)
	CERR << "// Peval conversion of var " << i << " " << CLOCK() << "\n";
      vals[i]=e2r(vals[i],lv1,contextptr);
    }
    if (debug_infolevel)
      CERR << "// Peval conversion to internal form completed " << CLOCK() << "\n";
...
#define CLOCKS_PER_SEC 1e6
  ...
  int mrref(const matrice & a, matrice & res, vecteur & pivots, gen & det,int l, int lmax, int c,int cmax,
    ...
    if (debug_infolevel>2)
      CERR << "// mrref reduction end:" << CLOCK() << "\n";
    if (algorithm!=RREF_GAUSS_JORDAN){
  ...
  void vranm(int n,const gen & F,vecteur & res,GIAC_CONTEXT){
    ...
    if (f.is_interval() && f._SYMBptr->feuille.type==_VECT){
    ...
    if (f.is_exp() || f.is_symb_of_sommet(at_randexp) || f.is_symb_of_sommet(at_exponential) || f.is_symb_of_sommet(at_exponentiald)){
      f=evalf_double(f._SYMBptr->feuille,1,contextptr);
      if (f.type!=_DOUBLE_ || f._DOUBLE_val<=0){
	res=vecteur(1,gensizeerr(contextptr));
	return;
  ...
  bool minv(const matrice & a,matrice & res,bool convert_internal,int algorithm,GIAC_CONTEXT){
    if (debug_infolevel)
      CERR << CLOCK() << " matrix inv begin" << "\n";
    matrice arref = a;
    add_identity(arref);
    if (debug_infolevel)
      CERR << CLOCK() << " identity added" << "\n";
    int s=int(a.size());
    gen det;
    vecteur pivots;
    ...
    if (!ok)
      return false;
    if (debug_infolevel)
      CERR << CLOCK() << " remove identity" << "\n";
    if (ok!=2 && !remove_identity(res,contextptr))
      return false;
    if (debug_infolevel)
      CERR << CLOCK() << " end matrix inv" << "\n";
    return true;
  }
  ...
  bool francis_schur(std_matrix<gen> & H,int n1,int n2,std_matrix<gen> & P,int maxiter,double eps,bool is_hessenberg,bool complex_schur,bool compute_P,bool no_lapack,GIAC_CONTEXT){
	...
	for (int j=0;j<i-1;j++){
      ...
      if (debug_infolevel>=2)
	CERR << CLOCK() << " qr iteration number " << niter << " " << "\n";
      if (debug_infolevel>=5)
	H.dbgprint();
  ...
  vecteur mpcar_int(const matrice & A,bool krylov,GIAC_CONTEXT,bool compute_pmin){
	...
	dbglevel=2;
      if (dbglevel>1)
	CERR << CLOCK() << " " << 100*pipd/testvalue << " % done" << (currentprob<proba?", stable.":", unstable.")<< "\n";
  ...
  bool egv(const matrice & m0,matrice & p,vecteur & d, GIAC_CONTEXT,bool jordan,bool rational_jordan_form,bool eigenvalues_only){
	...
	*logptr(contextptr) << "Warning! Automatic extension not implemented. You can try to diagonalize the matrix * a non trivial element of GF(" << modulo << ",lcm of degrees of factor(" << symb_horner(p_car,vx_var()) << "))" <<  "\n";
    ...
    for (;f_it!=f_itend;++f_it){
      ...
      //dbg_printf("egv factor=%s\n",gen(w).print(context0).c_str());
      int s=int(w.size());
      if (s<2)
	continue;
      if (s==2)
	...
	x=r2sym(*it,cur_lv,contextptr);
        //dbg_printf("egv lambda=%s mult=%i\n",x.print(context0).c_str(),n);
	...
	for (;;){
	  ...
	  if (n){
	    ...
	    cur_m=horner(cur_m_adjx,*it);
            //dbg_printf("egv cur_m=%s\n",cur_m.print(context0).c_str());
	    char_m=mergematrice(char_m,mtran(*cur_m._VECTptr));
...
#if defined EMCC || defined FXCG || defined TICE
