...
namespace giac {
  ...
  gen ext_reduce(const gen & a, const gen & v){
	...
	return algebraic_EXTension(a,v);
      //dbg_printf("ext_reduce %s mod %s\n",a.print().c_str(),v.print().c_str());
#if 1
	...
	if (a._VECTptr->size()==3){
	  ...
	  if (it==itend)
        ...
        //dbg_printf("ext_reduce result=%s,%s\n",r1.print().c_str(),r0.print().c_str());
	if (r1==0)
	  return r0;
  ...
  static void clean(gen & g){
    if (g.is_neg() && is_integer(g._SYMBptr->feuille))
...
#if 1
	gen c=new ref_vecteur;
        //dbg_printf("ext* a=%s b=%s\n",a._EXTptr->print().c_str(),b._EXTptr->print().c_str());
  ...
  */
  ...
  gen fminmax(const gen & g,int minmax,GIAC_CONTEXT){
    ...
    //dbg_printf("fminmax1 expr=%s var=%s\n",expr.print().c_str(),var.print().c_str());
	...
	if (varminmax.is_interval() && varminmax._SYMBptr->feuille.type==_VECT){
    ...
    if (do_find_range){
      ...
      //dbg_printf("fminmax after find_range %s\n",gen(range).print().c_str());
    ...
    //dbg_printf("fminmax recompute %s %s\n",resmin.print().c_str(),resmax.print().c_str());
    // Singularities
    vecteur ws=find_singularities(expr,*var._IDNTptr,0,contextptr);
    //dbg_printf("fminmax singu1 %s\n",gen(ws).print().c_str());
    int wss=int(ws.size());
    w.clear();
    for (int i=0;i<wss;++i){
    ...
    //dbg_printf("fminmax singu2 %s\n",gen(ws).print().c_str());
    recompute_minmax(w,range,expr,var,resmin,resmax,xmin,xmax,-1,contextptr);
    //dbg_printf("fminmax singu3 %s %s\n",resmin.print().c_str(),resmax.print().c_str());
    if (minmax/3){
      if (minmax %3 ==1)
	return xmin;
      if (minmax %3 ==2)
  ...
  static int insturmsign1(const gen & g0,bool strict,GIAC_CONTEXT){
    ...
    gen v0(v[0]);
    //dbg_printf("insturmsign g=%s v0=%s\n",g.print().c_str(),v0.print().c_str());
  ...
  int sturmsign(const gen & g0,bool strict,GIAC_CONTEXT){
    ...
    if (g.is_exp())
      return 1;
    /* if (g.is_pow() && g._SYMBptr->feuille[1]==plus_one_half)
       return 1; */
    if (g.is_prod()){
