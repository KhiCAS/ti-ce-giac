...
namespace giac {
  ...
  static gen derive_SYMB(const gen &g_orig,const identificateur & i,GIAC_CONTEXT){
    ...
    if (s.sommet==at_prod){
      bool do_step=step_infolevel(contextptr)>1 && count_noncst(s.feuille,i)>1;
      if (s.feuille.type==_VECT && s.feuille._VECTptr->size()==2 && s.feuille._VECTptr->back().is_inv() && !is_constant_wrt(s.feuille._VECTptr->back()._SYMBptr->feuille,i,contextptr)){
    ...
    if (s.sommet==at_fsolve && s.feuille.type==_VECT && s.feuille._VECTptr->size()>=2){
      ...
      if (v[1].is_equal() && v[1]._SYMBptr->feuille.type==_VECT && !v[1]._SYMBptr->feuille._VECTptr->empty())
  ...
  gen _grad(const gen & args,GIAC_CONTEXT){
    ...
    if (args._VECTptr->size()==3){
      ...
      if (opt.is_equal() && (opt._SYMBptr->feuille[0]==at_coordonnees || (opt._SYMBptr->feuille[0].type==_INT_ && opt._SYMBptr->feuille[0].val==_COORDS))){
