...
namespace giac {
  ...
  // symbolic symb_lin(const gen & a){    return symbolic(at_lin,a);  }
  ...
  gen _lin(const gen & args,GIAC_CONTEXT){
    ...
    if (is_algebraic_program(args,var,res))
      return symb_prog3(var,0,_lin(res,contextptr));
    if (is_equal(args))
  ...
  //symbolic symb_tlin(const gen & a){    return symbolic(at_tlin,a);  }
  ...
  gen _tlin(const gen & args,GIAC_CONTEXT){
    ...
    gen var,res;
    if (is_algebraic_program(args,var,res))
      return symb_prog3,makesequence(var,0,_tlin(res,contextptr));
    if (is_equal(args))
  ...
  gen sigma(const vecteur & v){
    // an "intelligent" version of return symb_plus(v);
  ...
  */
  ...
  gen prod_expand(const gen & a,const gen & b,GIAC_CONTEXT){
    ...
    if (!a_is_plus) // distribute wrt b
      return symb_plus(a*(*b._SYMBptr->feuille._VECTptr));
    if (!b_is_plus)
      return symb_plus(b*(*a._SYMBptr->feuille._VECTptr));
  ...
  static gen expand_pow_expand(const gen & e,GIAC_CONTEXT){
    ...
    if (v[1].is_plus() && v[1]._SYMBptr->feuille.type==_VECT){
    ...
    if (v[1].type==_INT_ ){
      if (v[0].is_prod()&& v[0]._SYMBptr->feuille.type==_VECT){
    ...
    if (v[0].is_plus() && v[0]._SYMBptr->feuille.type==_VECT && v[1].type==_INT_ && v[1].val>=0){
      int k=v[1].val;
      if (!k)
	return plus_one;
      if (k==1)
  ...
  gen exp_expand(const gen & e,GIAC_CONTEXT){
    ...
    if (e._SYMBptr->sommet==at_plus)
      return symb_prod(apply(e._SYMBptr->feuille,exp_expand,contextptr));
    gen coeff,arg;
  ...
  static gen ln_expand0(const gen & e,GIAC_CONTEXT){
    ...
    if (e._SYMBptr->sommet==at_prod)
      return _plus(apply(e._SYMBptr->feuille,ln_expand0,contextptr),contextptr);//symb_plus(apply(e._SYMBptr->feuille,ln_expand0,contextptr));
  ...
  //symbolic symb_texpand(const gen & a){    return symbolic(at_texpand,a);  }
  ...
  gen _texpand(const gen & args,GIAC_CONTEXT){
    ...
    gen var,res;
    if (is_algebraic_program(args,var,res))
      return symb_prog3(var,0,_texpand(res,contextptr));
    if (is_equal(args))
  ...
  gen expand(const gen & e,GIAC_CONTEXT){
    ...
    if (e.type!=_VECT && is_algebraic_program(e,var,res))
      return symb_prog3(var,0,expand(res,contextptr));
  ...
  //symbolic symb_expand(const gen & args){    return symbolic(at_expand,args);  }
  ...
  gen expexpand(const gen & e,GIAC_CONTEXT){
    ...
    if (is_algebraic_program(e,var,res))
      return symb_prog3(var,0,expexpand(res,contextptr));
  ...
  gen lnexpand(const gen & e,GIAC_CONTEXT){
    ...
    gen var,res;
    if (is_algebraic_program(e,var,res))
      return symb_prog3(var,0,lnexpand(res,contextptr));
  ...
  gen trigexpand(const gen & e,GIAC_CONTEXT){
    ...
    gen var,res;
    if (is_algebraic_program(e,var,res))
      return symb_prog3(var,0,trigexpand(res,contextptr));
    vector<const unary_function_ptr *> v;
    vector< gen_op_context > w;
    v.push_back(at_sin);
    w.push_back(&sin_expand);
